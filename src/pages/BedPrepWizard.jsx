...
import { BED_MAIN_TYPES, BED_SUB_TYPES, BED_FIELDS, getSubTypes } from "@/lib/bedPrepStages";
...
export default function BedPrepWizard() {
  ...
  async function handleSave() {
    setSaving(true);
    await base44.entities.Area.update(areaId, { bed_prep_data: JSON.stringify(data), status: "Complete" });
    setSaving(false);
    navigate(`/bed-prep-summary/${areaId}`);
  }
  ...
  function renderField(field) {
    ...
    return (
      <div key={field.key}>
        <Label>{field.label}</Label>
        <Input className="mt-1" value={data[field.key] || (field.defaultValue || "")} onChange={e => set(field.key, e.target.value)} />
      </div>
    );
  }
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-card border rounded-xl p-6 mb-6 space-y-5">
        {step === 0 && (
          <>
            <div>
              <h2 className="text-lg font-bold">Select Bed Preparation Type</h2>
              <p className="text-sm text-muted-foreground mt-1">Choose the main category for this bed preparation.</p>
            </div>
        ...
        {step === 1 && (
          <>
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">Choose the specific bed preparation method.</p>
            </div>
        ...
        {step === 2 && (
          <>
            <div>
              <h2 className="text-lg font-bold">Measurements &amp; Decisions</h2>
              <p className="text-sm text-muted-foreground mt-1">Enter all required measurements and confirm key decisions.</p>
            </div>
            {fields.measurements.length > 0 && (
              <div className="space-y-3">
                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground border-b pb-1">Measurements Needed</p>
                {fields.measurements.map(renderField)}
              </div>
            )}
            {fields.decisions.length > 0 && (
              <div className="space-y-3 mt-4">
                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground border-b pb-1">Decisions Needed</p>
                {fields.decisions.map(renderField)}
              </div>
            )}
          </>
        )}
        ...
        {step === 3 && (
          <>
            <div>
              <h2 className="text-lg font-bold">Constraints &amp; Risk Factors</h2>
              <p className="text-sm text-muted-foreground mt-1">Note any constraints, risk factors, or missing information.</p>
            </div>
            <div className="space-y-3">
              {fields.constraints.map(renderField)}
            </div>
          </>
        )}
      </div>
      ...
      <div className="flex justify-between">
        ...
        <Button
          ...
          disabled={saving || (step === 0 && !data.main_type) || (step === 1 && !data.sub_type)}
        >
