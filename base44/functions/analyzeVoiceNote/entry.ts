...
For Walkway/Patio operations, extract:
- patio_fields: object with all extracted form field values:
  time_estimate (hrs), vermeer_dingo_access ("Yes"/"No"/"Limited"), vermeer_lift_pallet ("Yes"/"No"), material_delivery ("Yes"/"No"/"With Restrictions"), access_measurements (text), existing_patio_reuse ("Yes"/"No"), demo_needed ("Yes"/"No"), demo_sf (number), demo_material (text), exc_needed ("Yes"/"No"), exc_patio_sf (number), exc_extra_perimeter_lf (number), exc_distance_to_truck (number), exc_machine_access ("Dingo"/"Vermeer"/"Skid Steer"/"No Machine Access"), exc_disposal_method (text), exc_by_hand_or_machine ("By Hand"/"By Machine (Dingo)"/"By Machine (Vermeer)"), exc_tight_access ("Yes"/"No"), exc_existing_material (text), base_selection ("3a"/"3b"/"3c"), base_sf (number), base_depth (number), base_lf_not_abutting (number), base_abutting_asphalt_cut ("Yes"/"No"), base_by_hand_or_machine ("By Hand"/"By Machine"), base_plate_compactor ("Yes"/"No"), level_selection ("4a"/"4b"/"4c"), level_sf (number), level_depth (number), install_selection ("5a"/"5b"/"5c"/"5d"), install_sf (number), install_lf_curves (number), install_pattern (text), install_material_type (text), install_border_course ("Yes"/"No"), edge_selection ("6a"/"6b"/"none"), edge_lf (number), joint_selection ("8a"/"8b"/"8c"/"8d"/"8e"/"8f"), joint_sf (number), repair_needed ("Yes"/"No"), seal_needed ("Yes"/"No"), notes (text)

For Hardscape - Repair Existing operations, extract:
- hr_fields: object with all extracted form field values:
  repair_type ("Patio or Walkway"), length (number), width (number), sf (number), time_estimate (hrs), material_type ("Brick"/"FlagStone - Dimensional"/"FlagStone - Irregular"/"Paver"), new_material_needed ("Yes"/"No"), new_material_unit ("SF"/"SY"/"LF"/"Each"), new_material_qty (number), new_base_needed ("Yes"/"No"), new_base_type ("Gator Base"/"Road Gravel Base"/"Permeable Base"), new_leveling_needed ("Yes"/"No"), new_leveling_type ("Limestone Screenings"/"Niagara Chip"/"Sand"), new_edge_needed ("Yes"/"No"), new_edge_type ("Aluminum Edge"/"Concrete Edge"), new_edge_lf (number), machine_access ("Vermeer"/"Dingo"/"By hand"), constraints (text), notes (text)

For Maintenance operations, extract:
- maintenance_type: One of "Weeding", "Core Aeration / Overseed", "Follow up Care - Planting Bed Maintenance", "Lawn", "Seasonal Clean Up", "Trees & Shrubs"
- maintenance_fields: object with all extracted form field values:
  time_estimate (hrs), length (number), width (number), sf (number), herbicide ("Yes"/"No"), herbicide_sf (number), estimated_hours (number), overseed_needed ("Yes"/"No"), seed_type (text), machine_needed ("Vermeer"/"Dingo"/"By hand"), visits (number), hours_per_visit (number), bed_size (text), travel_time (text), service_type (text), zones (number), season_scope ("Spring"/"Fall"), disposal_needed ("Yes"/"No"), debris_volume (text), plants (array of {name, count, size}), notes (text)

For Pathway / Steps operations, extract:
- ss_sub_type: "Stepping Stones" or "Hardscape - Steps"
  Choose "Stepping Stones" if notes mention stepping stones, pathway stones, or flagstone path.
  Choose "Hardscape - Steps" if notes mention steps, stair, or landing.
- ss_fields: object with all extracted form field values:
  time_estimate (hrs), lf (number), stone_count (number), stone_type (text), stone_size (text), existing_stones ("Yes"/"No"), releveling ("Yes"/"No"), additional_stones (number), existing_base (text), machine ("Vermeer"/"Dingo"/"By hand"), step_count (number), step_material (text), step_length (number), step_width (number), landing_needed ("Yes"/"No"), landing_length (number), landing_width (number), notes (text)

For Retaining Wall operations, extract:
- wall_type: One of "Outcrop", "Snapped", "Stone", "Timber", "Boulder", "CMU", "Boulder Slope Retention"
- rw_fields: object with all extracted form field values:
  time_estimate (hrs), exc_lf (number), exc_wall_height (number), exc_trench_depth (number), exc_trench_width (number), exc_disposal (boolean), exc_distance_to_truck (text), exc_spoil_type (text), exc_machine_access ("Vermeer"/"Dingo"/"By Hand"), base_lf (number), base_width (number), base_depth (number), base_fabric_sf (number), base_distance_to_truck (text), base_road_gravel_type (text), base_compactor ("Plate Compactor"/"Weber Jumping Jack"), wall_lf (number), wall_exposed_height (number), wall_corner_count (number), wall_fabric_sf (number), wall_distance_to_truck (text), wall_geogrid (boolean), wall_machine_access ("Vermeer"/"Dingo"/"By Hand"), boulder_type (text), boulder_size (text), boulder_count (number), notes (text)
          ...
          recommended_operations: {
            ...
            items: {
              ...
              properties: {
                ...
                patio_fields: { type: 'object', additionalProperties: true, description: 'For Walkway/Patio: all extracted field key-value pairs', properties: { time_estimate: {type:['number','null']}, notes: {type:'string'}, vermeer_dingo_access: {type:'string'}, vermeer_lift_pallet: {type:'string'}, material_delivery: {type:'string'}, access_measurements: {type:'string'}, existing_patio_reuse: {type:'string'}, demo_needed: {type:'string'}, demo_sf: {type:['number','null']}, demo_material: {type:'string'}, exc_needed: {type:'string'}, exc_patio_sf: {type:['number','null']}, exc_extra_perimeter_lf: {type:['number','null']}, exc_distance_to_truck: {type:['number','null']}, exc_machine_access: {type:'string'}, exc_disposal_method: {type:'string'}, exc_by_hand_or_machine: {type:'string'}, exc_tight_access: {type:'string'}, exc_existing_material: {type:'string'}, base_selection: {type:'string'}, base_sf: {type:['number','null']}, base_depth: {type:['number','null']}, base_lf_not_abutting: {type:['number','null']}, base_abutting_asphalt_cut: {type:'string'}, base_by_hand_or_machine: {type:'string'}, base_plate_compactor: {type:'string'}, level_selection: {type:'string'}, level_sf: {type:['number','null']}, level_depth: {type:['number','null']}, install_selection: {type:'string'}, install_sf: {type:['number','null']}, install_lf_curves: {type:['number','null']}, install_pattern: {type:'string'}, install_material_type: {type:'string'}, install_border_course: {type:'string'}, edge_selection: {type:'string'}, edge_lf: {type:['number','null']}, joint_selection: {type:'string'}, joint_sf: {type:['number','null']}, repair_needed: {type:'string'}, seal_needed: {type:'string'} } },
                hr_fields: { type: 'object', additionalProperties: true, description: 'For Hardscape Repair: all extracted field key-value pairs', properties: { repair_type: {type:'string'}, length: {type:['number','null']}, width: {type:['number','null']}, sf: {type:['number','null']}, time_estimate: {type:['number','null']}, notes: {type:'string'}, material_type: {type:'string'}, new_material_needed: {type:'string'}, new_material_unit: {type:'string'}, new_material_qty: {type:['number','null']}, new_base_needed: {type:'string'}, new_base_type: {type:'string'}, new_leveling_needed: {type:'string'}, new_leveling_type: {type:'string'}, new_edge_needed: {type:'string'}, new_edge_type: {type:'string'}, new_edge_lf: {type:['number','null']}, machine_access: {type:'string'}, constraints: {type:'string'} } },
                maintenance_type: { type: 'string', description: 'For Maintenance: "Weeding", "Core Aeration / Overseed", "Follow up Care - Planting Bed Maintenance", "Lawn", "Seasonal Clean Up", or "Trees & Shrubs"' },
                maintenance_fields: { type: 'object', additionalProperties: true, description: 'For Maintenance: all extracted field key-value pairs', properties: { time_estimate: {type:['number','null']}, notes: {type:'string'}, length: {type:['number','null']}, width: {type:['number','null']}, sf: {type:['number','null']}, herbicide: {type:'string'}, herbicide_sf: {type:['number','null']}, estimated_hours: {type:['number','null']}, overseed_needed: {type:'string'}, seed_type: {type:'string'}, machine_needed: {type:'string'}, visits: {type:['number','null']}, hours_per_visit: {type:['number','null']}, bed_size: {type:'string'}, travel_time: {type:'string'}, service_type: {type:'string'}, zones: {type:['number','null']}, season_scope: {type:'string'}, disposal_needed: {type:'string'}, debris_volume: {type:'string'}, plants: {type:'array',items:{type:'object',additionalProperties:true}} } },
                ss_sub_type: { type: 'string', description: 'For Pathway/Steps: "Stepping Stones" or "Hardscape - Steps"' },
                ss_fields: { type: 'object', additionalProperties: true, description: 'For Pathway/Steps: all extracted field key-value pairs', properties: { time_estimate: {type:['number','null']}, notes: {type:'string'}, lf: {type:['number','null']}, stone_count: {type:['number','null']}, stone_type: {type:'string'}, stone_size: {type:'string'}, existing_stones: {type:'string'}, releveling: {type:'string'}, additional_stones: {type:['number','null']}, existing_base: {type:'string'}, machine: {type:'string'}, step_count: {type:['number','null']}, step_material: {type:'string'}, step_length: {type:['number','null']}, step_width: {type:['number','null']}, landing_needed: {type:'string'}, landing_length: {type:['number','null']}, landing_width: {type:['number','null']} } },
                wall_type: { type: 'string', description: 'For Retaining Wall: "Outcrop", "Snapped", "Stone", "Timber", "Boulder", "CMU", or "Boulder Slope Retention"' },
                rw_fields: { type: 'object', additionalProperties: true, description: 'For Retaining Wall: all extracted field key-value pairs', properties: { time_estimate: {type:['number','null']}, notes: {type:'string'}, exc_lf: {type:['number','null']}, exc_wall_height: {type:['number','null']}, exc_trench_depth: {type:['number','null']}, exc_trench_width: {type:['number','null']}, exc_disposal: {type:'boolean'}, exc_distance_to_truck: {type:'string'}, exc_spoil_type: {type:'string'}, exc_machine_access: {type:'string'}, base_lf: {type:['number','null']}, base_width: {type:['number','null']}, base_depth: {type:['number','null']}, base_fabric_sf: {type:['number','null']}, base_distance_to_truck: {type:'string'}, base_road_gravel_type: {type:'string'}, base_compactor: {type:'string'}, wall_lf: {type:['number','null']}, wall_exposed_height: {type:['number','null']}, wall_corner_count: {type:['number','null']}, wall_fabric_sf: {type:['number','null']}, wall_distance_to_truck: {type:'string'}, wall_geogrid: {type:'boolean'}, wall_machine_access: {type:'string'}, boulder_type: {type:'string'}, boulder_size: {type:'string'}, boulder_count: {type:['number','null']} } }
              },
              required: ['operation_type', 'description']
            },
